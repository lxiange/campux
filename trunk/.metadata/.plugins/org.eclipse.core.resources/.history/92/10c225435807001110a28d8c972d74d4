...
{
	public static void lily_Get_ContentOfOneThemePage(String url, int num)throws IOException,ClassNotFoundException,SQLException
	{
	    //获取theme_page的格式化内容
		ArrayList<String> temp_arr=Lily_Content_Analyzer.lily_Content_Analyzer(url);
...
//    		content_Store_ToFile(temp_str,num);
	...
	public static boolean too_Long(ArrayList<String> topic)
	{
		...
		if((domain.length()>30)||(date.length()>30)||(title.length()>50)||(ip.length()>20)||(author.length()>50)||(from.length()>50))
			flag=true;
		return flag;
	...
	//对某主题页爬取到的内容的两种存储方式之二：数据库
	public static void content_Store_ToDB(ArrayList<String> topic)throws IOException,ClassNotFoundException,SQLException
	{
		...
		if(theme_or_reply=="f")
			...
			store_To_ReplyTable(url,domain,date,title,ip,author,content,from);
	}
	
	//根据内容中的'theme_or_reply'项的值，将theme_page的内容存入表'lily_theme'
	public static void store_To_ThemeTable(String url,String domain,String date,String title,String ip,String author,String content)throws IOException,ClassNotFoundException,SQLException
	{
		...
		Connection connection = DriverManager.getConnection("jdbc:sqlserver://localhost:1433;Databasename=lily","sa","123456");
//        Connection connection = DriverManager.getConnection("jdbc:sqlserver://192.168.1.5:1433;Databasename=lily","sa","shangshuBiz5_");
	...
	public static void store_To_ReplyTable(String url,String domain,String date,String title,String ip,String author,String content,String reply_from)throws IOException,ClassNotFoundException,SQLException
	{
...
//        Connection connection = DriverManager.getConnection("jdbc:sqlserver://192.168.1.5:1433;Databasename=lily","sa","shangshuBiz5_");
	...
	//将theme_page的内容存入文件
	public static void content_Store_ToFile(String source, int number)throws FileNotFoundException,IOException
	{
	...
	{
//        String url="http://bbs.nju.edu.cn/bbscon?board=IFA_IS&file=M.1240397056.A&num=33";
//		String url="http://bbs.nju.edu.cn/bbscon?board=NJUExpress&file=M.1316499236.A&num=16559";
//		String url="http://bbs.nju.edu.cn/bbscon?board=NJUExpress&file=M.1316502506.A&num=16562";
//		String url="http://bbs.nju.edu.cn/bbscon?board=AcademicReport&file=M.1270868965.A&num=6459";
		String url="http://bbs.nju.edu.cn/bbstcon?board=NJUExpress&file=M.1316443140.A";
//		String url="http://bbs.nju.edu.cn/bbscon?board=D_Maths&file=M.1316671294.A&num=11983";
