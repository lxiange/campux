 ...
 *postcondition: 若能从Document中识别出随机会话编号、组名， 则在表workgroup中建立相应的项，否则返回相应的错误响应的提示。
*/
...
public class User_Group_Append 
{
	public static String user_Group_Append (Document document,DynamicTable_RCL ran_con)
	{
		String response="";
		...
		//错误11：用户组已存在
		if(Connect_DBTable_WG.have_NameInWG(group_name)==true)
		{
			response=Connect_DBTable_WS.wrong_Response("11");
			return response;
		}
		else
		{
			String group_no=Connect_DBTable_WG.get_AutoNoForNewItem();
		
			Connect_DBTable_WG.append_ItemInWG(group_no, group_name);
