...
{
	public static ArrayList<String> lily_GetThemePageUrl_OfNewFramePage(String first_frame_page, int new_page,int page_increment)throws IOException,InterruptedException
	{
		ArrayList<String> all_theme_page=new ArrayList<String>();
		String aa="&start=";
		String domain=first_frame_page.substring(first_frame_page.indexOf("board=")+6);
		if(first_frame_page.contains(aa))
			domain=first_frame_page.substring(first_frame_page.indexOf("board=")+6,first_frame_page.indexOf("&start="));
		
		//获取new_page页目录页的url
		ArrayList<String> all_frame_page=Lily_GetFramePageUrl_FromSomePage.lily_GetFramePageUrl_FromSomePage(first_frame_page,new_page);
		//观察最后一页是否含有20个主题
		int residue=Lily_GetFramePageUrl_FromSomePage.lily_Judge_LastPage(first_frame_page);
		...
		if(all_frame_page.size()==1)
		{
			...
			ArrayList<String> temp_arr=Lily_GetThemePageUrl_OfOneFramePage.lily_GetThemePageUrl_OfOneFramePage(all_frame_page.get(all_frame_page.size()-1));
			while(temp_arr.size()==0)
			{
				System.out.println("Network connect again, please wait for 5s!");
				Thread.sleep(5000);
				temp_arr=Lily_GetThemePageUrl_OfOneFramePage.lily_GetThemePageUrl_OfOneFramePage(all_frame_page.get(all_frame_page.size()-1));
			}
		...
		else if(all_frame_page.size()<new_page&&residue!=0)
		{
			for(int i=0;i<all_frame_page.size()-1;i++)
			{
				...
				while(temp_arr.size()==0)
				{
					System.out.println("Network connect again, please wait for 5s!");
					Thread.sleep(5000);
					temp_arr=Lily_GetThemePageUrl_OfOneFramePage.lily_GetThemePageUrl_OfOneFramePage(all_frame_page.get(i));
				}
				for(int k=0;k<temp_arr.size();k++)
				    all_theme_page.add(temp_arr.get(k));
				Thread.sleep(400);
...
//    	String url="http://bbs.nju.edu.cn/bbsdoc?board=Mediastudy&start=265&type=doc";
    	...
    	ArrayList<String> all_theme_page=lily_GetThemePageUrl_OfNewFramePage(url,new_page,page_increment);
