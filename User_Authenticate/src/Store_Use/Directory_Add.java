 ...
 *postcondition: 若能从document中识别出随机会话编号、目录路径，并且表ran_con中有此编号，且其属于app组，则创建此目录，返回正确响应的提示，
 *               否则返回相应的错误响应的提示。
 */
package Store_Use;
...
public class Directory_Add 
{ 
	public static String directory_Add(Document document,DynamicTable_RCL ran_con,String absolute_path_prefix)
	{
		String response="";
		...
		//生成绝对路径名
		String absolute_path_of_sub_directory=absolute_path_prefix+sub_directory;
		File file=new File(absolute_path_of_sub_directory);
		//错误23：文件系统中已经有此目录名
		if(file.isDirectory()==true)
		{
			response=Connect_DBTable_WS.wrong_Response("26");
			return response;
		}	
		//错误27：无法创建子目录(由于系统等其他原因)
		File apr=new File(absolute_path_prefix+user_name_a);

		if(Directory_Function.add_SubDirectory(apr, sub_directory)==false)
		{
			response=Connect_DBTable_WS.wrong_Response("27");
			return response;
		}
