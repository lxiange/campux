...
//		if(end_index==len_end_str-1) return error_Happen(end_str);
 ...
 /*   public static boolean start_With(String source,String head)
    {
    	boolean flag=false;
    	int len=source.length();
    	int sub_len=head.length();
    	if(len<sub_len)return flag;
    	
    	String temp_str=source.substring(0,sub_len);
    	if(head.equals(temp_str))
    		flag=true;
