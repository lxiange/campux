...
{
	//获取某个frame_page下的所有theme_page的url，为进行第一次过滤后的结果(可能小于20个)
	public static ArrayList<String> lily_GetThemePageUrl_OfOneFramePage(String frame_url)throws IOException,InterruptedException
	{
		...
		{
			if(!pretreatment_1(all_theme_page.get(i)))
			{
				String temp_str=Fragment_Searcher.fragment_Searcher_WithoutBoundary(all_theme_page.get(i), "", ">");
...
/*		String file_name="lily_result/first_Filted.txt";
		...
		byte[] buf=("Filted pages num: "+not_good_page_count+"\r\n"+"Total pages num: "+(not_good_page_count+all_theme_page.size())+"\r\n").getBytes();
		out.write(buf);
		out.flush();
		out.close();		
*/		
		for(int i=0;i<all_theme_page.size();i++)
		{
			String str=Fragment_Searcher.fragment_Searcher_WithoutBoundary(all_theme_page.get(i),"","&num=");
			all_theme_page.set(i,str);
		}
		return all_theme_page;
	}
	//第一次过滤
	public static boolean pretreatment_1(String source)throws FileNotFoundException,IOException
	{
		boolean flag=false;
	//	String str_a="[公告]";
		String str_b="[合集]";
...
*/		}
	...
	public static ArrayList<String> lily_Get_TitlesOfOneMainPage(String url)throws IOException
	{
		...
		ArrayList<String> temp_arr=Fragment_Searcher.fragment_Searcher_All(source, "<a href=bbscon?", "</a>");

		for(int i=(temp_arr.size()-1);i>=0;i--)
		{
			String temp_str=Fragment_Searcher.fragment_Searcher_WithoutBoundary(temp_arr.get(i), "<a href=", " </a>");
	...
	{
     	String url="http://bbs.nju.edu.cn/bbsdoc?board=Olympics&start=0&type=doc";
